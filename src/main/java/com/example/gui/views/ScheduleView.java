...
public class ScheduleView extends SecuredView implements View {
  ...
  protected void prepareView() {
    ...
    NativeButton visitProfileButton = new NativeButton(
        String.format(String.format("Visit %s's profile", user.getName())), (Button.ClickListener) event -> {
    ...
    NativeButton seeSchedule = new NativeButton(String.format("Go to %s's schedule", user.getName()),
  ...
  private void prepareTrainingView() {
    ...
    if (placeToSee != null) {
      ...
      mapLayout.addComponent(currentUI
          .getGoogleMap(placeToSee.getLat(), placeToSee.getLng(), "Place of training: " + training.getCity(),
              "wiadomość"));
    ...
    double hour = training.getDate().getHours();
    double toMinutes = (minutes + (training.getLength() * 60 % 60));
    double toHour = hour + (training.getLength() * 60) - toMinutes;
  ...
  private void prepareAddTrainingWindow() {
    ...
    Button save = new Button(String.format("Save trainings"), (Button.ClickListener) event -> {
      ...
      if (hoursLength.getValue() == null || minutesLength.getValue() == null || (int) hoursLength
          .getValue() + (int) minutesLength.getValue() == 0) {
        Notification.show("Wrong data! ",
            Notification.Type.WARNING_MESSAGE);
        return;
      }
    ...
    for (Category cat : catManager.getUserCategories(loggedInUser)) {
    ...
    trainingDate.addValueChangeListener(event -> {
      if (trainingDate.getValue().before(new Date())) {
        trainingDate.setValue(new Date());
      }
    });
  ...
  private void prepareConfirmReservationWindow() {
    ...
    Button ok = new Button(String.format("Yes"), (Button.ClickListener) event -> {
      trainingManager.reserve(training, loggedInUser.getId());
      prepareSchedule();
      confirmReservation.setVisible(false);
      this.setEnabled(true);
    });
    ...
    for (int i = 0; i < (int) weeks.getValue(); i++) {
      for (int y = 0; y < 7; y++) {
        if (weekDays[y].getValue()) {
          ...
          training.getDate().setHours((int)hours.getValue());
          training.getDate().setMinutes((int) minutes.getValue());
          final double length = (int) hoursLength.getValue() + (int) minutesLength.getValue() / (double) 60;
          training.setLength(length);
          training.setOwnerId(loggedInUser.getId());
          final Category cat = (Category) category.getValue();
          ...
          if (!trainingManager.saveTraining(training, calendar.getTime())) {
            resultSet.add(calendar.getTime().toString());
          }
        }
  ...
  private void prepareTrainingsButtons() {
    ...
    for (int i = 1; i < weekDates.length + 1; i++) {
      ArrayList<Training> trainings = trainingManager.getTrainingsFromDate(weekDates[i - 1], user.getId());
      for (Training training : trainings) {
        double minute = training.getLength() % 1 * 60;
        double hour = training.getLength() - (minute / 60);
        NativeButton trainingButton = new NativeButton(String
            .format("%s :length: %.0f:%.0f", catManager.getCategory(training.getCategory()).getName(), hour, minute),
            (Button.ClickListener) event -> {
        ...
        daysLayouts[i].addComponent(trainingButton, String.format("top:%dpx;", (int) (140 * (training.getDate().getHours() - 6))));
      }
    }
