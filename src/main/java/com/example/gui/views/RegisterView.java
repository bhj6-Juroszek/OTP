...
public class RegisterView extends VerticalLayout implements View {
  ...
  private void init() {
    ...
    registerButton = new Button("Finish registration", (Button.ClickListener) event -> {
      if (registerPassword.getValue().equals(registerPassword2.getValue())) {
        if (!customerRep.existsAnother(registerMail.getValue(), "mail", (long) -1)) {
          if (!registerMail.getValue().equals("") && !registerLogin.getValue().equals("") && !registerPassword
              .getValue().equals("")) {
            if (name.getValue().equals("") || surname.getValue().equals("") || city.getValue().equals("") || postCode
                .getValue().equals("") || apartamentNumber.getValue().equals("")) {
    ...
    backButton = new Button("Back to mainView",
        (Button.ClickListener) event -> currentUI.getNavigator().navigateTo("mainView"));
