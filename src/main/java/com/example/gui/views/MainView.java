...
public class MainView extends AbsoluteLayout implements View{
    ...
    private void init() throws MySQLIntegrityConstraintViolationException {
     ...
     /*****************************************************************************************************************/
        ...
        Button registrationButton=new Button("Dont have account? click here!", (Button.ClickListener) event -> currentUI.getNavigator().navigateTo("registerView"));
        registrationButton.setStyleName(Reindeer.BUTTON_LINK);
        Button remindPasswordButton=new Button("You forgot you password ? Click here!", (Button.ClickListener) event -> window.setVisible(true) );
    ...
    private void prepareCustomersList(Category cat)
    {
        ...
        if(citySelect.getValue()!=null)cityName=citySelect.getValue().toString();
        double maxPrice = 0;
        double range=0;
        try {
        ...
        String sortBy=null;
        if(this.sortBy.getValue()!=null)sortBy=this.sortBy.getValue().toString();
            ArrayList<Training> trainings=dao.getTrainingsByFilters(cityName,range, cat, initialDate.getValue(), lastDate.getValue(), maxPrice,sortBy, showOnline.getValue());
        ...
        private void prepareCustomersOnPages(HorizontalLayout titlesLayout, ArrayList<Training> trainings, int number)
        {
            ...
            for(int i=number*10;i<number*10+10 && i<trainings.size();i++)
            {
                ...
                Label nameAndSurname=new Label(String.format("%s %s",training.getOwner().getName(),training.getOwner().getSurname()));
        ...
        private void navigate (String name)
        {
            ...
            {
                VaadinSession.getCurrent().setAttribute("customerToSee", VaadinSession.getCurrent().getAttribute("loggedInCustomer"));
